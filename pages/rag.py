...
def create_vector_store(pdfs):
    ...
    prompt = ChatPromptTemplate.from_template(
        """Answer the following question based only on provided context.<context>{context}</context>/n/nQuestion : {input}"""
    )
...
if st.session_state["rag_messages"] and len(st.session_state["rag_messages"]) > 0:
    for i in range(len(st.session_state["rag_messages"])):
        if st.session_state["rag_messages"][i]["role"] == "user":
            message(
                st.session_state["rag_messages"][i]["content"],
...
if "retrieval_chain" not in st.session_state:
    uploaded_files = st.file_uploader(
        "Choose a file", type=["pdf", "txt", "csv"], accept_multiple_files=True
    )
    ...
    if user_input and submit:
        output = st.session_state.retrieval_chain.invoke({"input": user_input})[
            "answer"
        ]
